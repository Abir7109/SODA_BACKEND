...
- `spotify_search` — opens Spotify Desktop and searches via Ctrl+L keyboard shortcut. No API keys needed.
- `spotify_play` — launches Spotify Desktop directly, searches via Ctrl+L, auto-plays first result. Uses keyboard automation (not URI protocol).
- `spotify_play_playlist` — searches for a playlist and plays it in Spotify Desktop.
...
- **Spotify env vars**: None required. Works with Spotify Free via desktop app + keyboard automation.
